 ...
 * @param creatorUserId
 * @text 作者のユーザーID
 * @desc RPGアツマールでの作者のユーザーIDを指定します。
 * @default 0
 * @type number
 *
 ...
 *
 * --------------------------------------
 * RPGアツマール上で、作者の共有セーブ領域をデータベースとして扱う（くらむぼんによる追加機能）
 * --------------------------------------
 * 1. プラグインパラメータの「作者のユーザーID」を指定する
 * 2. SheetJSをダウンロードしてプラグインとして有効にする（手順はすぐ上の「使用ライブラリ」を参照）
 * 3. 「書き出し手順」に則って、データベースをExcelに書き出す
 * 4. Excelファイルをプロジェクトフォルダの外に出し、アツマールにゲームをアップロードする
 * 5. （データベースの内容を変更したくなったら）3で書き出したExcelファイルの内容を書き換える
 * 6. アツマール上でゲームを開き、Excelファイルをゲーム画面にドラッグ＆ドロップする（作者アカウントでのログインが必須）
 * 7. 「（ファイル名）をデータベースとして作者の共有セーブに保存しますか？」と聞かれるので「はい」
 * 8. 保存が完了したら、右下に「サーバーにセーブを同期しました」と表示されるので、ページをリロードする
 *
 * ※現在対応している出力対象データは「データベース」のみです。（オリジナルデータベースも可）
 * ※現在対応している出力対象フォーマットは「xlsx,xlsm,xlsb,ods,fods」です。
 */
...
(function() {
    ...
    if (typeof Utils !== 'undefined' && Utils.RPGMAKER_NAME === 'MV') {
        ...
        if (!Utils.isOptionValid('test') && !DataManager.isEventTest()) {
            // アツマールで作者の共有セーブをデータベースとして読み込む
            const _Scene_Boot_isReady    = Scene_Boot.prototype.isReady;
            Scene_Boot.prototype.isReady = function() {
                if (_Scene_Boot_isReady.apply(this, arguments)) {
                    if (this._sharedSaveLoaded || !window.RPGAtsumaru || !(param.creatorUserId > 0)) {
                        return true;
                    }
                    if (this._sharedSaveLoaded === undefined) {
                        RPGAtsumaru.experimental.storage.getSharedItems([param.creatorUserId]).then(function(save) {
                            if (save[param.creatorUserId]) {
                                const json = JSON.parse(save[param.creatorUserId]);
                                for (let key in json) {
                                    window[key] = json[key];
                                }
                            }
                            this._sharedSaveLoaded = true;
                        }.bind(this));
                        this._sharedSaveLoaded = false;
                    }
                }
                return false;
            };

            // アツマールで作者がデータベースファイルをドラッグ＆ドロップすると共有セーブとして保存する
            if (window.RPGAtsumaru) {
                RPGAtsumaru.experimental.user.getSelfInformation().then(function(user) {
                    if (user.id === param.creatorUserId) {
                        ondragover = function (event) {
                            event.preventDefault();
                        };
                        window.ondrop = function(event) {
                            const file = event.dataTransfer.files[0];
                            document.body.innerHTML = `${file.name}をデータベースとして作者の共有セーブに保存しますか？`;
                            ...
                            button.onclick = function() {
                                ...
                                reader.onload = function() {
                                    const json = {};
                                    const databases = DataManager._databaseFiles.map(function(database) {
                                        return { VariableName: database.name, JsonName: database.src.replace(".json", "") };
                                    }).concat(param.targetDatabase);
                                    const target = new ConvertTargetDatabase(false);
                                    const workbook = XLSX.read(new Uint8Array(reader.result), { type: "array" });
                                    const serializer = new DataSerializer(target);
                                    target.execute(function(dataName) {
                                        const data = serializer.deserializeData(workbook.Sheets[dataName]);
                                        const key = databases.filter(function(databaseInfo) {
                                            return databaseInfo.JsonName === dataName;
                                        })[0].VariableName;
                                        json[key] = data;
                                    }.bind(this));
                                    localStorage.setItem("Atsumaru Shared", JSON.stringify(json));
                                    document.body.innerHTML = "送信しました。右下に「サーバーにセーブを同期しました」という<br>表示が出たことを確認してから、ページを更新してください";
                                }
